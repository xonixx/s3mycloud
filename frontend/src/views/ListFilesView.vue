...
async function load() {
  ...
  searchResults.value = response.data.page.map((e) => ({
    ...e,
    date: moment(e.uploaded).format("D MMM YYYY"),
  }));
...
function changePage(p: number) {
  ...
  router.push({
    path: "/",
    query: { ...router.currentRoute.value.query, page: p },
  });
  setTimeout(load, 10);
}
...
function changePageSize(ps: number) {
  ...
  router.push({
    path: "/",
    query: { ...router.currentRoute.value.query, pageSize: ps, page: 0 },
  });
  setTimeout(load, 10);
}
...
function changeQuery(q: string) {
  ...
  router.push({
    path: "/",
    query: { ...router.currentRoute.value.query, q, page: 0 },
  });
  setTimeout(load, 10);
}
</script>
...
<template>
  ...
  <div class="field is-grouped">
    <div class="control is-expanded">
      <input
        ref="qRef"
        :value="q"
        @keydown.enter="changeQuery(qRef.value)"
        class="input"
        type="text"
  ...
  <div v-if="totalRecords === 0">Nothing found</div>
  <div v-else>
